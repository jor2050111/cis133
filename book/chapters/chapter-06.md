...
3. Build the gallery row on the drive gallery page. Give that page's `main` a role-based hook (your Chapter 4 class-versus-id decision rule applies), then make it a wrapping, gapped flex row. Give the gallery's figures a width that seats two per line inside the readable column. The column offers 608 pixels of content width under border-box, so two figure widths plus one gap must fit inside that. Zero the gallery figures' old margins with a descendant selector so the gap owns the spacing.
