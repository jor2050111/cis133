...
Skills Lab 3A left Copperwind with a site that does everything right. Three pages validate with zero messages, every image carries honest alt text, and Devon's screen reader jumps region to region through the landmarks you built. Then Priya Sharma, the program's outreach coordinator, opened the site. She asked the question you could feel coming: "Our logo is teal. Our illustrations paint a whole desert palette. Why is the site gray?"

Priya has a point, and the answer is not that something went wrong. HTML says what content IS, and it stays silent about looks on purpose. Every color the site shows today is a stand-in nobody at Copperwind chose. This chapter opens with the question of where those looks come from. Changing them is the job of a second language, one you have known by name since Chapter 1. CSS describes how content LOOKS, and this chapter puts it in your hands: rules that select exactly the elements you mean and declare how they should appear.

The design team already did the hard part. Copperwind's design set (the logo, the charts, and the drive scenes) keeps one recorded palette. It ships in your data pack as a file of exact color values. You will connect stylesheets to pages three ways and learn which one professionals default to. You will write selectors that target whole regions, chosen groups, and single elements. You will dress all three pages from one file. And you will add a second validator to your toolkit, because CLO IV's critique habit covers styles as much as markup. One rule carries over from Chapter 3: nothing you style in this chapter may cost the meaning your landmarks built. Devon's jump menu works today, and it still works when the site is teal.
...
That is not a hypothetical payoff. It is the answer to Priya's question. The site is gray because nobody has written its looks yet. The fix will not open a single HTML file.
...
That value is a color written in a system Section 4.4 unpacks. For now, know that it is the deep teal from the Copperwind logo's shading. Load this rule and every `h1` on the page turns deep teal at once. Chapter 2 told you to stop picking heading levels for their looks and promised that CSS would own them. This rule is that promise starting to pay: the headings changed and not one tag did.
...
```css
/* Deep teal instead of copperwind teal: the darker shade keeps
...
How far can separation of concerns go? The classic demonstration is [CSS Zen Garden](https://csszengarden.com), a site that has collected designer submissions for over two decades. Every design on it uses the identical HTML file. Only the stylesheet changes, and the results range from clean corporate pages to full illustrated landscapes. Same content, same markup, any look. That is the power Copperwind's site is about to borrow at a smaller scale: one stylesheet, three pages, one visual identity.

The next exercise sends you hunting for the twist hiding inside Priya's complaint. Take the prediction seriously before you run it.
...
3. Priya approves a full redesign for the fall drive. Using separation of concerns, explain why the redesign can ship without editing a single HTML file.
...
Here is the reach that changes everything. Link this one file from all three Copperwind pages. One edit restyles the whole site. Add a fourth page next month and it inherits the site's look with a single link element. That one file follows the site everywhere: Chapter 8 adapts it to every screen size, and Chapter 12 publishes it along with the pages. When this book says "stylesheet" without qualification from here on, it means this.

One clarification keeps beginners out of trouble. The stylesheet does not broadcast itself to the site. Each page opts in with its own link element, so a three-page site carries three identical `<link>` lines, one per head. Forget one and that page alone greets visitors in browser defaults.
...
**Predict:** You will add an internal rule making the heading ink (`#333333`). Then you will add an inline style making it copperwind teal (`#268080`). With all three in place, which color shows? And after you delete the inline style, which of the two survivors takes over?
...
1. Copperwind adds a fourth page next month. With the site on one external stylesheet, list every edit the new page needs to wear the site's full look.
...
header {
    background-color: #268080;  /* copperwind teal */
...
Because `copperwind-styles.css` is linked from all three pages, these three rules style three headers, three navs, and two footers in one stroke. The contact page's missing footer stays deliberately absent. Notice what made this cheap: the semantic structure was already there. A page of anonymous `<div>` elements has nothing this clean to grab. In Chapter 6, that `nav` hook does more: its list of links becomes a real navigation bar.
...
Element selectors are all-or-nothing. Style `p` and every paragraph obeys. But Copperwind's pages have two kinds of paragraphs: ordinary prose and time-sensitive deadlines that should stand out. Same element, different roles. That calls for a **class selector**, which targets only the elements you have opted in.
...
### Fix It 4.1: The Rule That Never Arrives 🔧

A teammate is styling a gallery page. Its photo figures carry `class="gallery"`. They write this rule to lift the figures onto white panels:

```text
.galery {
    background-color: #ffffff;  /* white panels for the photo figures */
}
```

**Symptom:** The figures keep their plain background. Nothing explains why. No error appears in the browser. The stylesheet is legal CSS too. Hand it to the CSS validator waiting in Section 4.4. The report opens with the line a flawless file earns:

```text
Congratulations! No Error Found.
```

**Diagnose:** Inspect one figure with DevTools. The `.galery` rule appears nowhere in its Styles panel. The selector matches zero elements on the page. Here is the cause. A selector is a search. Its text must match the class attribute letter for letter. The rule hunts for `class="galery"`. The markup says `class="gallery"`. A one-letter miss is a total miss.

**Repair:** Spell the selector the way the markup spells the class:
...
**Verify:** Refresh the page. The white panels arrive. Inspect a figure. The `.gallery` rule now sits in its Styles panel. Bank the moral, because no validator will ever hand it to you. Valid CSS is not correct CSS. The validator checked that `.galery` is a legal selector. It is. Only you know the page spells it `gallery`.
...
2. A teammate names a class `.orange-box`. Copperwind rebrands to green next term. Explain what breaks, what does not, and what the class should have been named.
...
Every example so far has leaned on two properties. This section makes them official, gives you three ways to write their values, and hands Copperwind its palette. Then it closes the loop your Chapter 2 habit opened: a validator for your styles, and a lesson in what browsers do with CSS they cannot read.
...
Their limit is precision, and here Copperwind learns it the hard way. CSS's named `teal` is the exact color `#008080`. Copperwind's teal is `#268080`. Close, and close is not the brand.
...
```css
h2 {
    color: #268080;             /* Copperwind's exact brand teal */
}
...
Copperwind's palette ships in your pack as `assets/code/chapter-04/copperwind-palette.txt`. It lists eight site colors with exact hex values, a usage note apiece, and the passing contrast pairings. That file is the canon of the design set's colors. The pack's chart generator draws from the same values. So the site and its art always agree.
...
**Explain:** In 1-2 sentences, explain which digits let you rank the three without a screen, and check your ranking against `copperwind-palette.txt`'s names for these values.
...
* Selectors set the aim. An element selector styles every one of these. A class selector (`.deadline`, opted in through the `class` attribute) styles some of these by role. An id selector (`#drop-off-hours`, through the unique `id` attribute) styles exactly this one. A selector list (`h1, h2`) shares one block. Names describe the content's role, never its appearance.
* Colors arrive as names (quick but imprecise), hex values (`#268080`, the industry workhorse, read as red-green-blue pairs), or `rgb()` mixes. Copperwind's palette file carries the exact values and the text-on-background pairings that pass contrast, and pairings missing from its list have not earned that trust.
...
3. State the decision rule for choosing among element, class, and id selectors. Give one example of each from Copperwind's site.
4. From Chapter 2: List the parts of the required document skeleton, in order.
5. From Chapter 3: Name the four landmark elements and the page region each one marks.

---

## 4.6 Skills Lab 4A: Dress Copperwind's Site in Its Own Colors

**Goal:** Connect one external stylesheet to all three of Copperwind's pages and dress the site in the program's own palette with element, class, and id selectors. Validate markup and styles to zero messages.

**Dataset:** Provided files in `assets/code/chapter-04/` from the course data pack. `starter-site/` holds Copperwind's finished three-page site: `recycling-guide.html`, `drive-gallery.html`, `contact.html`, and its `images` folder. `copperwind-palette.txt` maps the design set's colors to exact hex values. It lists the text-on-background pairings that pass contrast. `skills-lab-4a-answers.txt` is your written answers file. The folder's README documents every file. Work at the extracted `cis133` root and style the site without changing its structure. Copy every hex value from the palette file instead of retyping it.
...
2. Create `copperwind-styles.css` at the folder's top level, beside the three pages. Start the file with a CSS comment carrying the lab number, your name, and the date.
3. Connect the stylesheet to all three pages: one `<link rel="stylesheet" href="copperwind-styles.css">` in each page's head.
...
2. Create one class with a role-based name and apply it where the content warrants on at least two pages. Copperwind's content offers candidates: a deadline, a key reminder, a callout.
...
1. One of the three pages carries exactly one leftover inline style. Hunt it down, move its declaration into a fitting rule in `copperwind-styles.css`, and delete the attribute from the markup. Record the hunt in answer 3.A, including why the stylesheet is the better home.
2. Validate all three pages with the W3C HTML validator and `copperwind-styles.css` with the CSS validator, repairing until every report shows zero messages. Log your results in answer 3.B, then add the critique no validator can make: name one thing about your styling that passes both validators but that you would still improve, and say why.
...
1. Cite the moment one edit to `copperwind-styles.css` changed all three pages at once: what did you edit, and what changed where? Then estimate what the same site-wide change would have cost on a site styled with inline styles on every element. Ground your answer in your own Part 1 and Part 2 work.
